...
After the preview body is rendered, ask the user using the platform's blocking question tool (`AskUserQuestion` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini):
...
When no blocking question tool is available, present numbered options and wait for the user's next reply.
...
- **No tracker available:** option C and Defer paths are not offered upstream (see `tracker-defer.md`), so the preview never fires in the File-tickets or LFG-with-Defer configurations. When LFG's plan contains Defer recommendations but the sink is unavailable, synthesis's tie-breaking rule (Stage 5 step 7b) downgrades those to Skip before the preview is rendered.
