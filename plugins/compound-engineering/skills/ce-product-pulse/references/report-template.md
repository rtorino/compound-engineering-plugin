...
- If the config carried strategy metrics forward (see `docs/product-pulse.md` "Strategy metrics carried forward"), render each one in the Usage section with its current value and delta. If a strategy metric has no current value, include it anyway and mark it `no data`.
...
---
_Source windows: analytics [{{start}} -> {{end}}], tracing [{{start}} -> {{end}}], payments [{{start}} -> {{end}}]. Trailing buffer: 15m. Saved to `~/pulse-reports/{{YYYY-MM-DD}}_{{HH-MM}}.md`._
~~~
